...
const loadAvatar = async (): Promise<void> => {
  ...
  if (props.version) {
    ...
    const checkResult = await avatarStore.seekCache(
      props.targetId,
      props.showStrategy,
      props.version,
    );
  ...
  try {
    console.log(
      "avatar.vue:头像信息:",
      [props.targetId, props.showStrategy, loadingUrl].join("-"),
    );
    const path = await avatarStore.getAvatar(
      props.targetId,
      props.showStrategy,
      loadingUrl,
    );
...
watch(
  ...
  async (newCacheItem) => {
    if (newCacheItem && !newCacheItem.loading && newCacheItem.localPath) {
      console.log(
        "avatar.vue:watch:头像缓存更新:",
        newCacheItem.localPath,
        "版本:",
        newCacheItem.version,
      );
