...
export const useBlackStore = defineStore('black', () => {
  ...
  const init = (): void => {
    ...
    unsubLoaded = (...args: unknown[]) => {
      const [, payload] = args as [Electron.IpcRendererEvent, { list: BlackItem[]; total: number }]
      list.value = payload.list
      page.total = payload.total
    }
    window.electronAPI.on('black:list:loaded', unsubLoaded)
    window.electronAPI.send('black:list:load', { pageNo: page.pageNo, pageSize: page.pageSize })
    console.log('blackStore 初始化完成')
  }

  const destroy = (): void => {
    if (unsubLoaded) window.electronAPI.removeListener('black:list:loaded', unsubLoaded)
    unsubLoaded = null
  }

  const reload = (pageNo = 1): void => {
    page.pageNo = pageNo
    window.electronAPI.send('black:list:load', { pageNo, pageSize: page.pageSize })
  }
