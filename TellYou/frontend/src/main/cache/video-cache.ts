...
class VideoCache {
  public beginServe(): void {
    ipcMain.handle('video:cache:get:original', async (event, params: { id: number}) => {
      try {
        const data = await messageDao.getExtendData(params) as VideoCacheItem
    ...
    ipcMain.handle('video:cache:get:thumbnail', async (event, params: { id: number}) => {
      try {
        const data = await messageDao.getExtendData(params) as VideoCacheItem
