...
class ImageCache {
  public beginServe(): void {
    ipcMain.handle('image:cache:get:original', async (event, params: { id: number}) => {
      try {
        const data = await messageDao.getExtendData(params) as ImageCacheItem
    ...
    ipcMain.handle('image:cache:get:thumbnail', async (event, params: { id: number}) => {
      try {
