...
public class PageResp<T> {
    ...
    public static <T> PageResp<T> init(Integer pageNo, Integer pageSize, Long totalRecords, Boolean isLast, List<T> list) {
        return new PageResp<>(pageNo, pageSize, totalRecords, isLast, list);
    }

    public static <T> PageResp<T> init(Integer pageNo, Integer pageSize, Long totalRecords, List<T> list) {
        return new PageResp<>(pageNo, pageSize, totalRecords, isLastPage(totalRecords, pageNo, pageSize), list);
    }

    public static <T> PageResp<T> init(IPage<T> page) {
        return init((int) page.getCurrent(), (int) page.getSize(), page.getTotal(), page.getRecords());
    }

    public static <T> PageResp<T> init(IPage page, List<T> list) {
        return init((int) page.getCurrent(), (int) page.getSize(), page.getTotal(), list);
    }

    public static <T> PageResp<T> init(PageResp resp, List<T> list) {
